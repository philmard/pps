...
double * dataset_generation(int numObjs, int numCoords, long *rank_numObjs)
{
    ...
    *rank_numObjs = (rank == size-1)? numObjs-(size-1)*numObjs/size : numObjs/size;
    ...
    if (rank == 0) {
         ...
         */
         for(i=0; i<size; ++i) {
                sendcounts[i] = (i == size-1)? numObjs-(size-1)*numObjs/size : numObjs/size; // objects
                sendcounts[i] *= numCoords; // elements to send
                displs[i] = (i*numObjs/size)*numCoords;
        }
    ...
    err = MPI_Bcast(sendcounts, size, MPI_INT, 0, MPI_COMM_WORLD);
    if(err != MPI_SUCCESS) {
        fprintf(stderr, "MPI_Bcast sendcounts failed with error code %d\n", err);
     ...
     */
    err = MPI_Scatterv(objects, sendcounts, displs, MPI_DOUBLE, rank_objects, sendcounts[rank], MPI_DOUBLE, 0, MPI_COMM_WORLD);
    if(err != MPI_SUCCESS) {
        fprintf(stderr, "MPI_Scatter objects failed with error code %d\n", err);
        MPI_Abort(MPI_COMM_WORLD, err);
    }
