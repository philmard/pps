...
int main(int argc, char **argv)
{
     ...
     */
    err = MPI_Bcast(clusters, numClusters * numCoords, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    if(err != MPI_SUCCESS) {
        fprintf(stderr, "MPI_Bcast initial cluster positions failed with error code %d\n", err);
    ...
    if (rank == 0) {
       ...
       for(i=0; i<size; ++i) {
                recvcounts[i] = (i == size-1)? numObjs-(size-1)*numObjs/size : numObjs/size;
                displs[i] = i*numObjs/size;
        }
    }
     ...
     */
     ...
     err = MPI_Bcast(recvcounts, size, MPI_INT, 0, MPI_COMM_WORLD);
     if(err != MPI_SUCCESS) {
        fprintf(stderr, "MPI_Bcast recvcounts failed with error code %d\n", err);
        MPI_Abort(MPI_COMM_WORLD, err);
     }
     err = MPI_Bcast(displs, size, MPI_INT, 0, MPI_COMM_WORLD);
     if(err != MPI_SUCCESS) {
        fprintf(stderr, "MPI_Bcast displs failed with error code %d\n", err);
        MPI_Abort(MPI_COMM_WORLD, err);
    }
    ...
    err = MPI_Gatherv(membership, recvcounts[rank], MPI_INT, tot_membership, recvcounts, displs, MPI_INT, 0, MPI_COMM_WORLD);
    if(err != MPI_SUCCESS) {
        fprintf(stderr, "MPI_Gather membership failed with error code %d\n", err);
        MPI_Abort(MPI_COMM_WORLD, err);
    }
